...
;
; @note 
;   All version of the IDL_IDLBridge before IDL 8.3 cause a memory leak when using asynchronous executions
;   See the reference IDL-68399 in the following link:
...
function cpu_process_manager::init, n_cores, debug=debug, multithread=multithread, output_path=output_path, wait_time=wait_time, verbose=verbose
  ...
  ;; create a script that will output the last 10 lines of each bridge log file 
  if keyword_set(debug) then begin
    debug_info=filepath('debug_info',root=output_path)
    openw, lun, debug_info, /get_lun
    printf, lun, '#!/bin/bash'
    printf, lun, 'file=`ls debug_bridge_*.txt`
    printf, lun, 'for i in ${file[@]}'
    printf, lun, 'do'
    printf, lun, '  echo "----------------------------"'
    printf, lun, '  echo ${i}'
    printf, lun, '  echo "----------------------------"'
    printf, lun, '  tail -10 ${i}'
    printf, lun, 'done'
    free_lun, lun
    file_chmod, debug_info, /a_execute
  endif
...
function cpu_process_manager::waitend
  ...
  if nn_w ge 1 then begin
    ...
    message, 'all bridges are idle and do not have an associated task'
...
;+
; Wait the end of execution of all tasks
;-
pro cpu_process_manager::waitall
...
;    p11, p12, p13, p14, p15, p16, p17, p18, p19, p20, _ref_extra=extra)
  ...
  ;; IDL 8.7 Keyword _REF_EXTRA is not allowed when calling a routine
  result=curr_bridge->getresult(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, $
    p11, p12, p13, p14, p15, p16, p17, p18, p19, p20, _extra=extra)
